...
static bool parse_opt(uint8_t *begin, uint8_t *buf, const uint8_t *end, uint8_t *name, uint8_t *len, uint8_t **next) {
	memcpy(name, "\0", 1);
	*len = (uint8_t) 0;
	while (true)
	{
		...
		uint16_t opt_code = htons(*((uint16_t *) buf));
		buf += 2;
		uint16_t opt_len = htons(*((uint16_t *) buf));
		buf += 2;
		if (buf + opt_len > end) {
			return false;
		}
		...
		if (opt_code == DNS_EDNS0_NSID) {
			*len = (uint8_t) opt_len;
			memcpy(name, buf, (size_t)opt_len);
		}
		if (opt_code == DNS_EDNS0_SUBNET) {
			uint16_t family = htons(*((uint16_t *) buf));
			*len = sprintf(name, " %d/%d", (int)(buf[2]), (int)(buf[3]));
		}
		buf += opt_len;
		if (buf >= end)
			break;
...
#else
    uint8_t *additional = aftername + 4;
    *((uint8_t *) (additional + 0)) = 0;                   // Root
    *((uint16_t *) (additional + 1)) = htons(DNS_REC_OPT); // EDNS0 OPT
    *((uint16_t *) (additional + 3)) = htons(4096);        // UDP Payload size
    *((uint8_t *) (additional + 5)) = 0;                   // Extended rcode
    *((uint8_t *) (additional + 6)) = 0;                   // EDNS0 version
    *((uint16_t *) (additional + 7)) = htons(0);           // Z
    *((uint16_t *) (additional + 9)) = htons(23);           // Length
    *((uint16_t *) (additional + 11)) = htons(DNS_EDNS0_SUBNET); // Opt
    *((uint16_t *) (additional + 13)) = htons(7);          // Opt Len
    *((uint16_t *) (additional + 15)) = htons(1);          // Family (IPv4)
    *((uint16_t *) (additional + 17)) = 0x18;              // Netmask (24)
    *((uint16_t *) (additional + 18)) = 0x00;              // Scope (0)
    *((uint16_t *) (additional + 19)) = 0x7f;              // Client subnet (127.200.33.0)
    *((uint16_t *) (additional + 20)) = 0xc8;
    *((uint16_t *) (additional + 21)) = 0x21;
    *((uint16_t *) (additional + 22)) = htons(DNS_EDNS0_COOKIE); // Opt
    *((uint16_t *) (additional + 24)) = htons(8);                // Opt len
    int r = rand();
    *((uint16_t *) (additional + 26)) = (r >> 24) & 0xff;
    *((uint16_t *) (additional + 27)) = (r >> 16) & 0xff;
    *((uint16_t *) (additional + 28)) = (r >> 8) & 0xff;
    *((uint16_t *) (additional + 29)) = (r >> 0) & 0xff;
    *((uint16_t *) (additional + 30)) = (r >> 16) & 0xff;
    *((uint16_t *) (additional + 31)) = (r >> 24) & 0xff;
    *((uint16_t *) (additional + 32)) = (r >> 0) & 0xff;
    *((uint16_t *) (additional + 33)) = (r >> 8) & 0xff;
    *((uint16_t *) (buffer + 10)) = htons(0x0001);        // Additional
    return additional + 34 - buffer;
#endif
}
...
static ssize_t dns_question_create(uint8_t *buffer, char *name, dns_record_type type, uint16_t id)
{
    ...
    dns_buffer_set_id(buffer, id);     // ID
    *((uint16_t *) (buffer + 2)) = 0;  // Flags
    *((uint16_t *) aftername) = htons(type);              // Post (Type)
    *((uint16_t *) (aftername + 2)) = htons(DNS_CLS_IN);  // Post (Class)
    *((uint16_t *) (buffer + 4)) = htons(0x0001);         // Questions
...
bool dns_parse_record(uint8_t *begin, uint8_t *buf, const uint8_t *end, uint8_t **next, dns_record_t *record)
{
    ...
    else if (record->type == DNS_REC_OPT)
    {
        if (!parse_opt(begin, record->data.raw, end, record->data.name.name, &record->data.name.length, NULL))
        {
            return false;
        }
    }
...
char* dns_raw_record_data2str(dns_record_t *record, uint8_t *begin, uint8_t *end)
{
    ...
    switch(record->type)
    {
        ...
        case DNS_REC_OPT:
	    name.length = 0;
            parse_opt(begin, record->data.raw, end, name.name, &name.length, NULL);
            dns_print_readable(&ptr, sizeof(buf), name.name, name.length);
            break;
        case DNS_REC_MX:
            if(record->length < 3)
